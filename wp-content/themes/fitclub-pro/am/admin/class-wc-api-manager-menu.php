...
class API_Manager_Fitclub_theme_MENU {

	private $api_manager_theme_fitclub_key;

	// Load admin menu
	public function __construct() {

		$this->api_manager_theme_fitclub_key = AMET()->api_manager_theme_fitclub_key;
	...
	public function wc_am_api_key_field() {
		...
		if ( AMET()->ame_options[AMET()->ame_api_key] ) {
			echo "<span class='icon-pos'><img src='" . AMET()->theme_url() . "am/assets/images/complete.png' title='' style='padding-bottom: 4px; vertical-align: middle; margin-right:3px;' /></span>";
		} else {
			echo "<span class='icon-pos'><img src='" . AMET()->theme_url() . "am/assets/images/warn.png' title='' style='padding-bottom: 4px; vertical-align: middle; margin-right:3px;' /></span>";
		}
	}
	...
	public function validate_options( $input ) {
		...
		if ( $_REQUEST['option_page'] != AMET()->ame_deactivate_checkbox ) {
			...
			if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $api_email == '' || $checkbox_status == 'on' || $current_api_key != $api_key  ) {
				...
				$activate_results = json_decode( $this->api_manager_theme_fitclub_key->activate( $args ), true );
	...
	public function replace_license_key( $current_api_key ) {
		...
		$reset = $this->api_manager_theme_fitclub_key->deactivate( $args ); // reset license key activation
	...
	public function wc_am_license_key_deactivation( $input ) {
		...
		// $activate_results = json_decode( $this->api_manager_theme_fitclub_key->status( $args ), true );
		...
		if ( $options == 'on' && $activation_status == 'Activated' && AMET()->ame_options[AMET()->ame_api_key] != '' && AMET()->ame_options[AMET()->ame_activation_email] != '' ) {
			...
			$activate_results = json_decode( $this->api_manager_theme_fitclub_key->deactivate( $args ), true );
			...
			if ( $activate_results['deactivated'] == true ) {
				...
				add_settings_error( 'wc_am_deactivate_text', 'deactivate_msg', __( 'Theme license deactivated. ', AMET()->text_domain ) . "{$activate_results['activations_remaining']}.", 'updated' );
	...
	public function css_scripts() {
		...
		wp_register_style( AMET()->ame_data_key . '-css', AMET()->theme_url() . 'am/assets/css/admin-settings.css', array(), AMET()->version, 'all');
		wp_enqueue_style( AMET()->ame_data_key . '-css' );
	}

}

new API_Manager_Fitclub_theme_MENU();
