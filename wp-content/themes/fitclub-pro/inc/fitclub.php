...
.posted-on:hover .date,.navigation .nav-links a:hover,.bttn:hover, button:hover, input[type="button"]:hover,input[type="reset"]:hover,input[type="submit"]:hover,.sub-toggle,#site-navigation ul.sub-menu li.current-menu-item >a,#site-navigation ul.sub-menu li.current-menu-ancestor > a,#site-navigation ul.sub-menu > li:hover > a, .tg-tribe-events-list-widget .tribe-events-widget-link:hover,,#tribe-events .tribe-events-button:hover, .tribe-events-button:hover{
	...
	if( get_theme_mod( 'fitclub_site_title_font', 'Open Sans' ) != 'Open Sans' ) {
		$fitclub_internal_css .= ' #site-title a { font-family: "'.get_theme_mod( 'fitclub_site_title_font', 'Open Sans' ).'"; }';
	}
	if( get_theme_mod( 'fitclub_site_tagline_font', 'Open Sans' ) != 'Open Sans' ) {
		$fitclub_internal_css .= ' #site-description { font-family: "'.get_theme_mod( 'fitclub_site_tagline_font', 'Open Sans' ).'"; }';
	}
	if( get_theme_mod( 'fitclub_primary_menu_font', 'Open Sans' ) != 'Open Sans' ) {
		$fitclub_internal_css .= ' .main-navigation li, .site-header .menu-toggle { font-family: "'.get_theme_mod( 'fitclub_primary_menu_font', 'Open Sans' ).'"; }';
	}
	if( get_theme_mod( 'fitclub_widget_titles_font', 'Open Sans' ) != 'Open Sans' ) {
		$fitclub_internal_css .= ' .main-title { font-family: "'.get_theme_mod( 'fitclub_widget_titles_font', 'Open Sans' ).'"; }';
	}
	if( get_theme_mod( 'fitclub_other_titles_font', 'Open Sans' ) != 'Open Sans' ) {
		$fitclub_internal_css .= ' h1, h2, h3, h4, h5, h6 { font-family: "'.get_theme_mod( 'fitclub_other_titles_font', 'Open Sans' ).'"; }';
	}
	if( get_theme_mod( 'fitclub_content_font', 'Open Sans' ) != 'Open Sans' ) {
		$fitclub_internal_css .= ' body, button, input, select, textarea, p, blockquote p, .entry-meta, .more-link { font-family: "'.get_theme_mod( 'fitclub_content_font', 'Open Sans' ).'"; }';
	} // End of Google Fonts Custom CSS

	// Font Size Options
	$fitclub_fonts_size_custom = fitclub_font_size_func();

	foreach ($fitclub_fonts_size_custom as $fitclub_font) {

		$fitclub_font_size_setting = $fitclub_font['fitclub_font_size_setting'];
		foreach ($fitclub_font_size_setting as $fitclub_font_size_css) {

			if( get_theme_mod( $fitclub_font_size_css['id'], $fitclub_font_size_css['default'] ) != $fitclub_font_size_css['default'] ) {
			$fitclub_internal_css .= $fitclub_font_size_css['custom_css'] . '{ font-size: '.get_theme_mod( $fitclub_font_size_css['id'], $fitclub_font_size_css['default'] ).'px; }';
			}
		}
	}// End of Font Size Options

	// Color Options
	$fitclub_color_custom = fitclub_color_func();

	foreach ($fitclub_color_custom as $fitclub_color) {

		$fitclub_color_settings = $fitclub_color['fitclub_color_settings'];
		foreach ($fitclub_color_settings as $fitclub_color_css) {

			if( get_theme_mod( $fitclub_color_css['id'], $fitclub_color_css['default'] ) != $fitclub_color_css['default'] ) {
			$color_code = array_key_exists( 'color_location', $fitclub_color_css ) ? 'background-color: ' : 'color: ';
			$fitclub_internal_css .= $fitclub_color_css['color_custom_css'] . '{' . $color_code .get_theme_mod( $fitclub_color_css['id'], $fitclub_color_css['default'] ).'; }';
			}
		}
...
/* Register shortcodes. */
add_action( 'init', 'fitclub_add_shortcodes' );
	/**
	...
	*/
	function fitclub_add_shortcodes() {
	/* Add theme-specific shortcodes. */
		add_shortcode( 'the-year', 'fitclub_the_year_shortcode' );
		add_shortcode( 'site-link', 'fitclub_site_link_shortcode' );
		add_shortcode( 'wp-link', 'fitclub_wp_link_shortcode' );
		add_shortcode( 'tg-link', 'fitclub_themegrill_link_shortcode' );
	}
	...
	* @return string
	*/
	function fitclub_the_year_shortcode() {
		return date( 'Y' );
	}
	...
	* @return string
	*/
	function fitclub_site_link_shortcode() {
	...
	* @return string
	*/
	function fitclub_wp_link_shortcode() {
		return '<a href="' .esc_url( 'http://wordpress.org' ). '" target="_blank" title="' . esc_attr__( 'WordPress', 'fitclub' ) . '"><span>' . esc_html__( 'WordPress', 'fitclub' ) . '</span></a>';
	}

	/**
	* Shortcode to display a link to themegrill.com.
	*
	* @return string
	*/
	function fitclub_themegrill_link_shortcode() {
		return '<a href="' .esc_url( 'http://themegrill.com/themes/fitclub-pro/' ) .'" target="_blank" title="'.esc_attr__( 'ThemeGrill', 'fitclub' ).'" rel="designer"><span>'.esc_html__( 'ThemeGrill', 'fitclub') .'</span></a>';
	}
	...
	*/
	...
	function fitclub_footer_copyright_info() {
		...
		$default_footer_value = '<span class="copyright-text">' . get_theme_mod( 'fitclub_footer_editor', esc_html__( 'Copyright &copy; ', 'fitclub' ).'[the-year] [site-link]. All rights reserved. '.esc_html__( 'Design by ', 'fitclub' ).'[tg-link]. '.esc_html__( 'Powered by ', 'fitclub' ).'[wp-link].' ) . '</span>';

		$fitclub_footer_copyright = '<div class="copyright">'.$default_footer_value.'</div>';
		echo do_shortcode( $fitclub_footer_copyright );
	}
	endif;
...
function fitclub_header_title() {
	if( is_archive() ) {
		if( fitclub_is_tribe_page() ){
			...
			$fitclub_header_title = woocommerce_page_title();
		}
		else {
...
add_filter('loop_shop_columns', 'fitclub_loop_columns');
if (!function_exists('fitclub_loop_columns')) {
	function fitclub_loop_columns() {
...
add_filter( 'woocommerce_output_related_products_args', 'fitclub_related_products_args' );
  function fitclub_related_products_args( $args ) {
...
add_filter( 'excerpt_length', 'fitclub_excerpt_length' );
/**
 * Sets the post excerpt length to 40 words.
 *
 * function tied to the excerpt_length filter hook.
 * Checks for user input in customizer.
 * Sets default to 40 if user input is not available.
 *
 * @uses filter excerpt_length
 */
function fitclub_excerpt_length( $length ) {
	$excerpt_length = get_theme_mod( 'fitclub_excerpt_words', '40' );
